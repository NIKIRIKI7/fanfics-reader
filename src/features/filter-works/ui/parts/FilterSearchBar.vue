...
// Props to control UI state from parent
defineProps<{
...
<template>
  <div class="bg-background-primary/80 backdrop-blur-md border border-border rounded-xl p-4 shadow-sm flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
    ...
    <div class="flex items-center gap-4 w-full lg:w-auto justify-between lg:justify-end">
      ...
      <div v-if="!showArchived" class="bg-background-tertiary p-1 rounded-lg flex">
        <button 
          v-for="filter in mainFilters"
          :key="filter"
          @click="store.setType(filter as any)"
          :class="[
            'px-4 py-1.5 rounded text-sm font-sans font-medium transition-all duration-300',
            activeType === filter 
      ...
      <!-- Archive Indicator -->
      <div v-else class="flex items-center gap-2 text-text-muted text-sm font-sans px-4">
