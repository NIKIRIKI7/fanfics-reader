...
<template>
  <article class="group relative flex flex-col rounded-xl border border-transparent hover:border-border bg-background-tertiary/20 hover:bg-background-tertiary/40 transition-all duration-300 overflow-hidden h-full">
    ...
    <div class="relative w-full h-64 shrink-0 bg-background-secondary overflow-hidden">
      ...
      <!-- Gradient -->
      <div class="absolute inset-0 bg-gradient-to-b from-black/60 via-transparent to-transparent pointer-events-none opacity-80 group-hover:opacity-60 transition-opacity"></div>

      <!-- Type Badge -->
      <div class="absolute top-3 left-3 z-30 px-2 py-1 rounded bg-black/40 backdrop-blur-md text-white border border-white/20 text-[10px] font-bold uppercase tracking-wider shadow-sm">
        {{ work.type }}
      </div>
    ...
    <div class="flex-1 flex flex-col p-6">
      ...
      <!-- Title -->
      <h3 class="text-2xl font-bold text-text-primary mb-3 font-display leading-tight decoration-text-muted transition-all">
        <RouterLink :to="`/work/${work.slug}`" class="group-hover:underline underline-offset-4 decoration-1 decoration-accent before:absolute before:inset-0 z-0 focus:outline-none">
          {{ work.title }}
        </RouterLink>
      </h3>
      ...
      <div class="flex flex-wrap items-center gap-3 mb-2 text-xs font-sans relative z-10">
        ...
        <span :class="work.stats.status === 'Complete' ? 'text-green-500' : 'text-text-secondary'" class="font-bold">
          {{ work.stats.status }}
        </span>
      </div>

      <!-- Warnings (Restored Style) -->
      <div v-if="work.warnings?.length" class="mb-4 text-xs font-sans text-text-muted relative z-10 leading-relaxed">
        <!-- Лейбл -->
      ...
      <!-- Summary -->
      <p class="text-text-secondary line-clamp-3 mb-5 font-light text-base font-display leading-relaxed">
        {{ work.summary }}
      </p>
