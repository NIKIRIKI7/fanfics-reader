  ...
  <header class="sticky top-0 z-50 w-full border-b border-border bg-background-primary/95 backdrop-blur-sm transition-colors duration-300">
    <div class="max-w-[1200px] mx-auto px-6 h-16 flex items-center justify-between">
      ...
      <RouterLink to="/" class="flex items-center gap-3 group cursor-pointer">
        ...
        <h1 class="text-sm md:text-base font-bold tracking-tight uppercase text-text-primary font-display group-hover:tracking-wide transition-all duration-300">
      ...
      <nav class="hidden md:flex items-center gap-8">
        ...
        >
          {{ link.label }}
          <!-- Animated Underline -->
          <span class="nav-line absolute bottom-0 left-0 w-full h-[2px] bg-accent rounded-full opacity-0 origin-center scale-x-0 will-change-transform"></span>
        </RouterLink>
      </nav>

      <!-- Mobile Toggle -->
      <button @click="isMenuOpen = !isMenuOpen" class="md:hidden p-2 text-text-primary transition-transform active:scale-90">
    ...
    <!-- Mobile Menu -->
    <transition
      @enter="(el, done) => gsap.fromTo(el, { height: 0, opacity: 0 }, { height: 'auto', opacity: 1, duration: 0.3, onComplete: done })"
      @leave="(el, done) => gsap.to(el, { height: 0, opacity: 0, duration: 0.2, onComplete: done })"
    >
      <div v-if="isMenuOpen" class="md:hidden border-t border-border bg-background-primary px-6 py-4 flex flex-col gap-4 overflow-hidden">
        <RouterLink
          v-for="link in links"
          :key="link.path"
          :to="link.path"
