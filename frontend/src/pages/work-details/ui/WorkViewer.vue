...
const handleScroll = () => { if (scrollTimeout) return; scrollTimeout = window.setTimeout(() => { progressStore.saveProgress(props.work.slug, currentChapter.value, window.scrollY); scrollTimeout = null; }, 500); };
const nextChapter = async () => { if (currentChapter.value < props.work.stats.chapters) { currentChapter.value++; await nextTick(); scrollToContentStart(); progressStore.saveProgress(props.work.slug, currentChapter.value, 0); } };
const prevChapter = async () => { if (currentChapter.value > 1) { currentChapter.value--; await nextTick(); scrollToContentStart(); progressStore.saveProgress(props.work.slug, currentChapter.value, 0); } };
const toggleChapterList = () => { isChapterListOpen.value = !isChapterListOpen.value; };
const selectChapter = async (chapter: number) => { if (chapter !== currentChapter.value) { currentChapter.value = chapter; await nextTick(); scrollToContentStart(); progressStore.saveProgress(props.work.slug, currentChapter.value, 0); } isChapterListOpen.value = false; };
const handleKeydown = (e: KeyboardEvent) => {
  ...
  if (e.key === 'Escape') { if (isChapterListOpen.value) isChapterListOpen.value = false; else if (settingsStore.activeMenuId) settingsStore.closeSettings(); else if (isFocusMode.value) settingsStore.setFocusMode(false); }
};
const touchStartX = ref(0); const touchStartY = ref(0); const touchEndX = ref(0); const touchEndY = ref(0);
const handleTouchStart = (e: TouchEvent) => { const touch = e.changedTouches[0]; if (touch) { touchStartX.value = touch.screenX; touchStartY.value = touch.screenY; } };
const handleTouchEnd = (e: TouchEvent) => { const touch = e.changedTouches[0]; if (touch) { touchEndX.value = touch.screenX; touchEndY.value = touch.screenY; handleSwipe(); } };
const handleSwipe = () => { const diffX = touchStartX.value - touchEndX.value; const diffY = touchStartY.value - touchEndY.value; if (Math.abs(diffX) > 50 && Math.abs(diffY) < 100) { if (diffX > 0) nextChapter(); else prevChapter(); } };
const checkProgress = () => { const saved = progressStore.getProgress(props.work.slug); if (saved && (saved.chapter > 1 || saved.scroll > 200)) { savedProgressState.value = saved; showResumePrompt.value = true; setTimeout(() => showResumePrompt.value = false, 10000); } };
const resumeReading = async () => { if (!savedProgressState.value) return; const { chapter, scroll } = savedProgressState.value; currentChapter.value = chapter; showResumePrompt.value = false; await nextTick(); setTimeout(() => { window.scrollTo({ top: scroll, behavior: 'instant' }); }, 50); };
const closePrompt = () => { showResumePrompt.value = false; };
onMounted(() => { if (viewerContentRef.value) contentElement.value = viewerContentRef.value.contentElement; checkProgress(); historyStore.addWork(props.work); window.addEventListener('scroll', handleScroll, { passive: true }); window.addEventListener('keydown', handleKeydown); window.addEventListener('touchstart', handleTouchStart, { passive: true }); window.addEventListener('touchend', handleTouchEnd, { passive: true }); });
onUnmounted(() => { window.removeEventListener('scroll', handleScroll); window.removeEventListener('keydown', handleKeydown); window.removeEventListener('touchstart', handleTouchStart); window.removeEventListener('touchend', handleTouchEnd); if (scrollTimeout) clearTimeout(scrollTimeout); settingsStore.setFocusMode(false); settingsStore.closeSettings(); });
</script>
