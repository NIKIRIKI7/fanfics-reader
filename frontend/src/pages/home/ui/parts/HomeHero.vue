...
<template>
  <div class="relative flex flex-col items-center justify-center min-h-[85vh] px-6">
    ...
    <div class="fixed top-20 right-0 w-[400px] h-[400px] rounded-full bg-accent/5 blur-[120px] pointer-events-none z-0"></div>

    <!-- Используем ref для контейнера анимации -->
    <div ref="containerRef" class="relative z-10 flex flex-col items-center gap-8 max-w-2xl text-center">
      <!-- Avatar -->
      <div class="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden border border-border shadow-2xl transition-all duration-500 hover:scale-105 grayscale hover:grayscale-0">
        <img
      ...
      <!-- Header -->
      <div>
        <h2 class="text-4xl md:text-6xl font-display italic font-light tracking-tight text-text-primary mb-2">
          Author Pseudonym
        </h2>
        <p class="text-text-muted text-lg font-light font-display italic">est. 2023</p>
      </div>
      <!-- Quote -->
      <p class="text-text-secondary text-lg md:text-xl font-normal leading-relaxed font-display border-l-2 border-border pl-4 italic">
