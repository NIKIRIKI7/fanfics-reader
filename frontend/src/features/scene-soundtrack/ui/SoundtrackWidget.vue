    ...
    <transition :css="false" @enter="onEnterAccordion" @leave="onLeaveAccordion">
      ...
      >
        ...
        >
          ...
          <div
            v-if="isTrackActive(track.id) && isPlaying"
            class="flex items-end gap-[2px] h-3 ml-2"
          >
...
.animate-music-bar-2 {
  animation: music-bar 0.8s infinite ease-in-out 0.1s;
}
.animate-music-bar-3 {
  animation: music-bar 0.5s infinite ease-in-out 0.2s;
}
</style>
