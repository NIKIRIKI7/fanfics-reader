...
const classes = computed(() => {
  const base = 'group relative flex items-center justify-center rounded-full transition-colors duration-300 border cursor-pointer select-none overflow-hidden';
...
<template>
  <button ref="buttonRef" :class="classes" @click="toggle" :title="isSaved ? 'Remove from Library' : 'Save to Library'">
    ...
    <span v-if="variant === 'full'" class="ml-2 text-xs font-bold uppercase tracking-wider hidden sm:inline">
