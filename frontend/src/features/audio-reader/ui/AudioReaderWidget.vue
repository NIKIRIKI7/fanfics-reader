      ...
      <transition :css="false" @enter="onEnter" @leave="onLeave">
        ...
        >
          ...
          <div v-if="authorAudioUrl" class="flex p-1 bg-background-tertiary rounded-lg">
            <button @click="selectMode('tts')" class="flex-1 py-1 text-[10px] font-bold uppercase tracking-wider rounded transition-all duration-300" :class="activeMode === 'tts' ? 'bg-background-primary text-text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary'">Neural TTS</button>
            <button @click="selectMode('author')" class="flex-1 py-1 text-[10px] font-bold uppercase tracking-wider rounded transition-all duration-300" :class="activeMode === 'author' ? 'bg-background-primary text-text-primary shadow-sm' : 'text-text-muted hover:text-text-secondary'">Author Record</button>
          </div>
          ...
          <div class="flex items-center justify-between border-b border-border/50 pb-2">
            <div class="flex items-center gap-2">
              ...
              <span class="text-[10px] font-bold uppercase tracking-widest text-text-muted">{{ activeMode === 'author' ? 'Source: File' : 'Source: Neural' }}</span>
            </div>
            <button @click="store.stop()" class="text-text-muted hover:text-red-500 transition-colors" title="Terminate"><span class="material-symbols-outlined text-[18px]">stop_circle</span></button>
          </div>

          <div class="flex items-center justify-center gap-6 py-2">
            <button @click="togglePlay" class="w-14 h-14 rounded-full bg-accent text-background-primary flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-lg shadow-accent/20">
              <span class="material-symbols-outlined text-[32px] ml-1">{{ isPlaying ? 'pause' : 'play_arrow' }}</span>
            </button>
          </div>
          ...
          <div class="h-8 flex items-center justify-center gap-[3px] bg-background-tertiary/30 rounded-lg overflow-hidden px-2 relative">
            ...
            <div v-for="i in 16" :key="i" class="w-1 bg-accent rounded-full transition-all duration-100" :class="isPlaying ? 'animate-wave' : 'h-[2px] opacity-30'" :style="{ animationDelay: `${i * 0.05}s` }"></div>
          </div>
          ...
          <div class="space-y-3 pt-2 border-t border-border/50">
            <div v-if="activeMode === 'tts' && isTtsSupported" class="flex flex-col gap-1">
              ...
              <div class="relative">
                <select :value="selectedVoiceURI" @change="store.setVoice(($event.target as HTMLSelectElement).value)" class="w-full bg-background-tertiary border border-border rounded px-2 py-1.5 text-xs text-text-primary outline-none focus:border-accent font-sans appearance-none">
            ...
            <div class="flex flex-col gap-1">
              ...
              <input type="range" min="0.5" max="2" step="0.1" :value="rate" @input="store.setRate(Number(($event.target as HTMLInputElement).value))" class="w-full h-1 bg-background-tertiary rounded-lg appearance-none cursor-pointer accent-accent">
            </div>
          </div>
        </div>
      </transition>
...
@keyframes wave { 0%, 100% { height: 10%; opacity: 0.5; } 50% { height: 90%; opacity: 1; } }
</style>
