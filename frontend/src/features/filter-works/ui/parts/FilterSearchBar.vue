...
<template>
  <div class="bg-background-primary/80 backdrop-blur-md border border-border rounded-xl p-4 shadow-sm flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center">
    <!-- Search Input -->
    <div class="w-full lg:w-1/3 relative group">
      <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-text-primary transition-colors">
        search
      </span>
      <input
        v-model="searchQuery"
    ...
    <!--
      ...
      устраняя наложение.
    -->
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full lg:w-auto sm:justify-between lg:justify-end">

      <!-- Type Tabs (Visible only if NOT in Archive) -->
      <!-- Добавлен justify-center для мобилок -->
      <div v-if="!showArchived" class="bg-background-tertiary p-1 rounded-lg flex justify-center sm:justify-start">
        <button
      ...
      <!-- Archive Indicator -->
      <div v-else class="flex items-center justify-center sm:justify-start gap-2 text-text-muted text-sm font-sans px-4">
      ...
      >
        ...
        <span v-if="activeFiltersCount > 0" class="flex items-center justify-center w-5 h-5 bg-accent text-background-primary text-[10px] font-bold rounded-full">
          {{ activeFiltersCount }}
        </span>
      </button>
    </div>
